...
function getDayName(date) {
  ...
  return objDays[new Date(date).getDay()];
}
...
function formatDate(date) {
  ...
  const hours = data.getUTCHours() === 12 ? 12 : data.getUTCHours() % 12;
  const minutes = `0${data.getMinutes()}`.slice(-2);
  const seconds = `0${data.getSeconds()}`.slice(-2);
  const partOfDay = data.getUTCHours() / 12 >= 1 ? 'PM' : 'AM';

  return `${month}/${day}/${year}, ${hours}:${minutes}:${seconds} ${partOfDay}`;
}
...
function getWeekNumberByDate(date) {
  let answer = 0;
  const startWeek = new Date(date.getFullYear(), 0, 1);
  while (startWeek.getDay() !== 1) {
    if (!answer) answer = 1;
    startWeek.setDate(startWeek.getDate() + 1);
  }

  if (date < startWeek) return 1;

  while (date >= startWeek) {
    answer += 1;
...
function getNextFridayThe13th(date) {
  const answer = new Date(date);
  answer.setDate(13);
  if (answer > date && answer.getDay() === 5) return answer;

  answer.setMonth(answer.getMonth() + 1);
